...
Page({
  ...
  */
  ...
  //同意
  handleOpenAgree() {
    if (this.data.changeCount == 1) {
      return false;
    }
    this.setData({
      changeVisible: true,
      changeVisibleMsg:'同意更换',
      changeVisibleMsgDetail:'您是否同意更换成该阿姨？',
      changeClick:'agree'
    });
  },
  //同意
  agree: function ({ detail }) {
    var that = this;
    if (!this.isLogin()) {
      this.login();
      return false;
  ...
  handleOpenNotAgree() {
    ...
    this.setData({
      changeVisible: true,
      changeVisibleMsg: '拒绝更换',
      changeVisibleMsgDetail: '您是否拒绝更换？',
      changeClick: 'notAgree'
    });
  },
  //不同意
  notAgree: function ({ detail }) {
    var that = this;
    if (!this.isLogin()) {
      this.login();
      return false;
    ...
    if (detail.index === 0) {
      ...
      var data = {
        receiver: that.data.staffId,
        housekeeperId: that.data.housekeeperId,
        orderNo: that.data.orderNo
      }
      util.doPut("/weiXin/sendNotify/recommendRefuse", data, function (res) {
        //console.log(res)
        if (res) {
          that.setData({
            changeType: true,
